...
<Page actionBarHidden="true" backgroundSpanUnderStatusBar="true"> 
    <StackLayout> 
        ...
        <GridLayout columns="60,52,*" rows="50,10,50"> 
            <Label row="0" col="1" id="buttonCamera" :text="'\uf083'"
            @tap="takePicture" class="fas take-picture-icon "></Label> 
            <Label row="0" col="2" text="Take a picture" class="desc-text"/> 
            <Label row="2" col="1" id="buttonImage" :text="'\uf1c5'" 
            @tap="chooseImage" class="fas take-picture-icon "></Label> 
...
export default { 
    ...
    mounted() { 
        ...
        .then( 
            //request permissions for camera success 
            () => { 
                //have permissions 
            }, failure => { 
    ...
    methods: { 
        takePicture() { 
            ...
            .then(picture => { //save to file 
                imageSourceModule.fromAsset(picture)
                .then( savedImage => { 
                    console.log("saving to file"); 
                    let filename = this.$store.state.profile.id + "-" + new Date().getTime() + ".jpg";
                    let folder = knownFolders.documents(); 
                    let fullpath = path.join(folder.path, filename); 
                    savedImage.saveToFile(fullpath, "jpeg"); //set the picture from the currently saved image path 
        ...
        saveProfilePic() { 
            ...
            if (this.pictureSource != this.origSource) { 
                ...
                firebase.storage .uploadFile({ 
                    ...
                    remoteFullPath: "uploads/profile_pic/" + filename, // option 2: a full file path (ignored if 'localFile' is set) 
                    localFullPath: this.pictureSource, // get notified of file upload progress 
